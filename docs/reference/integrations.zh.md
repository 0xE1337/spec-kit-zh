...
下表的"命令目录"列列出的是每个集成安装其命令或技能所用的目录。上下文文件的定位与集成的多安装安全性是两个独立的问题：`multi_install_safe` 是集成对命令/技能路径的声明，而可选的 agent-context 扩展管理的是各智能体独立的上下文文件（例如 `AGENTS.md` 或 `CLAUDE.md`），甚至可以通过其 `context_files` 设置一次同步多个锚点。在那里，多个智能体映射到同一个上下文文件是正常预期，并不影响某个集成是否多安装安全；详见 agent-context 扩展。
