...
Specify CLI 支持范围广泛的 AI 编码智能体。运行 `specify init` 时，CLI 会为你选择的 AI 编码智能体配置相应的命令文件、上下文规则和目录结构——无论你偏好哪个工具，都能立即开始规范驱动开发。
...
显示所有可用的集成、当前安装的是哪个，以及每个集成需要 CLI 工具还是基于 IDE。
