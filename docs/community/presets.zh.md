...
| 预设 | 用途 | 提供 | 依赖 | URL |
|--------|---------|----------|----------|-----|
| A11Y Governance | 添加 WCAG 2.2 AA 治理、无障碍的文本/JSON 状态一致性、德英双语交付、CEFR-B2 可读性、包容性内容、教学式注释审查，以及可供审计的证据 | 10 个模板、3 个命令 | — | [spec-kit-preset-a11y-governance](https://github.com/hindermath/spec-kit-preset-a11y-governance) |
| Agent Parity Governance | 添加共享指引与生成命令的一致性、机群完成证据、无密钥的运行器/状态元数据、可供审计的证据，以及与智能体无关的模型路由。 | 6 个模板、3 个命令 | — | [spec-kit-preset-agent-parity-governance](https://github.com/hindermath/spec-kit-preset-agent-parity-governance) |
...
| Architecture Governance | 添加安全软件架构、可续跑的远程事务边界、STRIDE+CAPEC 威胁建模、arc42 安全横切概念、S-ADR、零信任适用性、OWASP SAMM 治理、BSI C3A 云自主性、BSI C5 云合规保障，以及可供审计的 Spec Kit 运行证据 | 13 个模板、3 个命令 | — | [spec-kit-preset-architecture-governance](https://github.com/hindermath/spec-kit-preset-architecture-governance) |
...
| Cross-Platform Governance | 添加 Bash/PowerShell 与只读检查的对等实现、根路径与原生覆盖审查、Unix man 手册页、双语帮助、Verb-Noun 命名纪律，以及可供审计的证据。 | 8 个模板、3 个命令 | — | [spec-kit-preset-cross-platform-governance](https://github.com/hindermath/spec-kit-preset-cross-platform-governance) |
...
| Intake Authoring Governance | 从有序的文本来源创建可追溯的 Spec Kit 入库文件，现在还能如实采用遗留的入库文件，而不会杜撰前置回执。 | 7 个模板、2 个命令、2 个脚本 | — | [spec-kit-preset-intake-authoring-governance](https://github.com/hindermath/spec-kit-preset-intake-authoring-governance) |
...
| iSAQB Architecture Governance | 添加 iSAQB/CPSA-F 与 arc42 架构治理，包括覆盖目标、视图、可续跑性、部分失败场景、ADR、风险和技术债的可供审计的证据。 | 13 个模板、3 个命令 | — | [spec-kit-preset-isaqb-architecture-governance](https://github.com/hindermath/spec-kit-preset-isaqb-architecture-governance) |
...
| Security Governance | 添加内存安全语言与安全编码治理、精确 HEAD 与安全关卡证据、供应商失败分类、ASVS、供应链透明度，以及欧盟法规筛查。 | 14 个模板、3 个命令 | — | [spec-kit-preset-security-governance](https://github.com/hindermath/spec-kit-preset-security-governance) |
